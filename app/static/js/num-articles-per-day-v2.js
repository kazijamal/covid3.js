d3.csv('/data/sentiment/publicmedia', function (day_data) {
  ...
  day_data.forEach(function (d, i) {
    d.numArticles = parseInt(d.numArticles);
  });
  ...
  function create_time_unit_data(parse_date) {
    ...
    new_data.forEach(function (d, i) {
      ...
      if (offset == 0 || i == 0 || i == day_data.length - 1) {
        ...
        new_data[tar_idx].numArticles += d.numArticles;
      ...
      if (d.start == false) {
        ...
        for (var idx = i; idx >= 0; idx--) {
          if (data_array[idx].start == true) {
            prev_val = data_array[idx].numArticles;
            prev_dist = i - idx;
            break;
          }
        }
        ...
        for (var idx = i; idx < data_array.length; idx++) {
          if (data_array[idx].start == true) {
            next_val = data_array[idx].numArticles;
            next_dist = idx - i;
            break;
          }
        }
        ...
        d.numArticles =
          prev_val +
          (next_val - prev_val) * (prev_dist / (prev_dist + next_dist));
      }
    });
  }
  ...
  function updateChart() {
    ...
    var lineGenerator = d3.svg
      ...
      .y(function (d) {
        return y(d.numArticles);
      });

    var lines = chart.selectAll('.line').data([curr_data]);

    lines.enter().append('path').attr('class', 'line').attr('d', lineGenerator);

    lines.transition().duration(1000).attr('d', lineGenerator);
  ...
  function mousemove() {
    ...
    var tar_value = curr_data[dIdx].numArticles;
    focus.attr(
      'transform',
      'translate(' + x(formatDate.parse(tar_date)) + ',' + y(tar_value) + ')'
    );
    ...
    tooltip
      ...
      .style(
        'left',
        d3.mouse(this)[0] - tooltip[0][0].clientWidth / 2.0 + 'px'
      );
  }
